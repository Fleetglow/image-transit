...
function Tracking() {
  useEffect(() => {
    const key = "gpt-image-visitor";
    let visitorId = localStorage.getItem(key);
    if (!visitorId) {
      visitorId = crypto.randomUUID();
      localStorage.setItem(key, visitorId);
    }
    fetch("/api/track", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ visitorId, path: window.location.pathname, referrer: document.referrer, userAgent: navigator.userAgent }), keepalive: true }).catch(() => undefined);
  }, []);
  return null;
}
  ...
  function renderTable(title: string, billing: string, index: string) {
    ...
    return <section className={`section table-section color-block ${billing === "按量计费" ? "block-lime" : "block-lilac"}`} key={billing}>
      <div className="wrap">
        ...
        <div className="table-shell"><div className="table-scroll"><table><thead><tr><th>名称 / 注册网址</th><th><button className={`sort-button ${sortKey === "rating" ? "active" : ""}`} onClick={() => toggleSort("rating")}>推荐 {sortLabel("rating")}</button></th><th><button className={`sort-button ${sortKey === "price" ? "active" : ""}`} onClick={() => toggleSort("price")}>价格 {sortLabel("price")}</button></th><th>备注</th></tr></thead><tbody>{rows.map((channel) => <tr key={channel.name}><td><span className="channel-name">{channel.name}</span><a className="channel-url" href={channel.url} target="_blank" rel="noreferrer">{channel.url}</a></td><td><span className="rating">{channel.rating}.0 <span className="rating-bar"><span style={{ width: `${channel.rating * 10}%` }} /></span></span></td><td className="price">{channel.price}</td><td className="note">{channel.note}</td></tr>)}</tbody></table>{rows.length === 0 && <div className="empty">没有匹配的渠道</div>}</div></div>
      </div>
    </section>;
  }
